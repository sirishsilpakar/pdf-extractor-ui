...
import { History, X, Download, ExternalLink } from "lucide-react";
...
export function RunsPanel({ runs, pagination, onViewResults, onLoadRunLog, onPageChange, isLoading }: Props) {
  const [viewingLog, setViewingLog] = useState<{ id: string, content: string } | null>(null);
    ...
    return (
      <div className="space-y-4">
        <div className="flex items-center gap-2 px-1">
          ...
          <h2 className="text-lg font-semibold tracking-tight">Extraction History</h2>
        </div>
        <div className="glass rounded-2xl p-12 flex flex-col items-center justify-center gap-4 border-2 border-dashed border-border/50">
          ...
          <p className="font-medium text-muted-foreground">No runs yet. Start an extraction to see run history.</p>
        </div>
      </div>
    );
  }
          ...
          <div className="glass-strong w-full max-w-4xl max-h-[85vh] rounded-2xl border border-border/50 flex flex-col overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between px-6 py-4 border-b border-border/30 bg-secondary/10">
              <div className="flex items-center gap-3">
                <Badge variant="outline" className="bg-primary/10 text-primary font-mono text-[10px]">LOG</Badge>
                <h3 className="font-semibold text-sm">Activity Log for Run {viewingLog.id.slice(0, 8)}…</h3>
              </div>
              <div className="flex items-center gap-2">
                <Button 
                  ...
                  onClick={() => {
                    const blob = new Blob([viewingLog.content], { type: 'text/plain' });
                ...
                <Button variant="ghost" size="icon" className="h-8 w-8 rounded-xl" onClick={() => setViewingLog(null)}>
                  <X className="h-4 w-4" />
                </Button>
              </div>
            </div>
      ...
      <div className="space-y-3">
        {isLoading ? (
          Array.from({ length: 3 }).map((_, i) => (
            <div key={`skeleton-${i}`} className="glass rounded-2xl border border-border/50 p-4 space-y-4">
          ...
          runs.map((run) => {
            ...
            const statusInfo = {
              running: { label: "Processing", color: "bg-primary/10 text-primary border-primary/30" },
              ...
              cancelled: { label: "Cancelled", color: "bg-yellow-500/10 text-yellow-500 border-yellow-500/30" },
            }[run.status] ?? { label: run.status.toUpperCase(), color: "bg-secondary text-muted-foreground" };

            return (
              <div key={run.id} className="glass rounded-2xl border border-border/50 p-4 space-y-3">
                {/* Card header */}
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    ...
                    <Badge variant="outline" className={cn("rounded-lg text-xs font-semibold", statusInfo.color)}>
                      {statusInfo.label}
                    </Badge>
                  </div>
                ...
                <div className="grid grid-cols-4 gap-3">
                  {[
                    { label: "Duration", value: fmtDuration(run.elapsedSeconds) },
                    { label: "Files", value: `${run.completedFiles} / ${total}`, sub: failed > 0 ? `${failed} failed` : undefined },
                    { label: "Direct", value: String(direct) },
                    { label: "OCR", value: String(ocr) },
                  ].map(s => (
                  ...
                  <div className="flex justify-between text-[10px] font-medium px-0.5">
                    <div className="flex gap-3">
                      <span className="text-blue-500 flex items-center gap-1">
                        <span className="w-1.5 h-1.5 rounded-full bg-blue-600" /> Direct {directPct}%
                      </span>
                      ...
                      {failedPct > 0 && (
                        <span className="text-destructive flex items-center gap-1">
                          <span className="w-1.5 h-1.5 rounded-full bg-destructive" /> Failed {failedPct}%
                        </span>
                      )}
                    </div>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    variant="ghost"
                    size="sm"
                    className="rounded-xl gap-2 flex-1 sm:flex-none text-muted-foreground"
      ...
      {pagination.total > 0 && (
        <div className="flex items-center justify-between px-2 py-4">
          <p className="text-xs text-muted-foreground">
            Showing page {pagination.page} of {pagination.pages} ({pagination.total} runs)
          </p>
          <div className="flex items-center gap-2">
            <Button
